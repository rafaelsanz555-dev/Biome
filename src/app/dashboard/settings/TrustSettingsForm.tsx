        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex items-center gap-3 border-t border-[#171512]/10 pt-4">
                <button
                    type="submit"
                    disabled={saving}
                    className="px-5 py-2.5 rounded-lg bg-[#C9A84C] hover:bg-[#D8BA63] text-[#0D0D0D] font-bold text-sm transition disabled:opacity-50"
                >
                    {saving ? 'Guardando...' : 'Guardar'}
                </button>
                {saved && <span className="text-sm text-[#D8BA63]">✓ Guardado</span>}
            </div>
        </form>
    )
}
