...
export default async function NotFound() {
    ...
    return (
        <div className="min-h-screen bg-[#0A0B0E] flex flex-col items-center justify-center p-6 selection:bg-green-500/40 selection:text-white">
            ...
            <div className="relative z-10 text-center space-y-6 mb-12">
                ...
                <p className="text-zinc-400 max-w-md mx-auto leading-relaxed">
                    ...
                    <br />
                    <span className="text-zinc-500 text-sm">Tip: los usernames van sin guiones bajos (ej: <code className="text-green-400 font-mono">anareyes</code>, no <code className="text-red-400 font-mono">ana_reyes</code>)</span>
                </p>
                        ...
                        {suggested.map((c) => {
                            ...
                            return (
                                <Link
                                    key={c.username}
                                    href={`/${c.username}`}
                                    className="group p-4 rounded-xl border border-gray-800 bg-[#0F1114] hover:border-green-500/50 hover:bg-green-500/5 transition text-center"
                                >
                                    <div className="w-12 h-12 rounded-full mx-auto mb-3 bg-gradient-to-br from-green-500 to-emerald-700 flex items-center justify-center text-white font-bold overflow-hidden">
                                        {c.avatar_url ? (
                                            <img src={c.avatar_url} alt="" className="w-full h-full object-cover" />
                                        ) : initial}
                                    </div>
                                    <p className="text-sm font-bold text-white truncate group-hover:text-green-400 transition">
                                        {c.full_name || c.username}
                                    </p>
                                    <p className="text-[11px] text-gray-500 truncate">@{c.username}</p>
                                    <ArrowRight size={11} className="mx-auto mt-2 text-gray-600 group-hover:text-green-400 transition" />
                                </Link>
                            )
                        })}
                    </div>
